...
- **API 文档**: 完整 RESTful API 文档（`API.md`），统一错误码体系
- **静默心跳**: 前后端自动刷新，数据实时同步
...
- 服务器默认监听 `0.0.0.0:3000`，支持局域网公网访问
- 自动检测本机 IP 地址，自动适配 `BASE_URL`
- 支持 `BASE_URL` 环境变量手动覆盖

**API 访问日志**

- 新增 `requestLogger` 中间件，实时记录所有 API 请求
- 新增 `api_logs` 数据库表，持久化存储访问日志
- 日志内容：请求方法、URL、状态码、IP、User-Agent、响应时间
- 控制台实时输出 `[API Log]` 方便调试

**后台仪表盘增强**

- 存储用量实时显示（所有图片文件大小总和）
- 浏览量排行改为「存储用量趋势」折线图（最近7天）
- Y 轴自动转换单位（B/KB/MB），tooltip 详细显示
- 数据真实驱动，非假数据
- 静默心跳机制：每 30 秒自动刷新，数据变更实时同步

**图片管理增强**

- 支持公开上传（无需认证），适合前台用户使用
- 多图上传：限制从 1 个文件增加到 5 个文件
- 批量删除：同步清理上传文件和缩略图文件
- 上传日志：前端添加 debug 日志方便排查问题

**完整 RESTful API 体系**

- 分类管理：增删改查、批量删除、权限验证、图片关联检查
- 图片管理：上传、删除、批量删除、资源文件清理
- 统一错误处理：完整错误码体系（400xxx-500xxx）
- JWT 认证 + 角色权限控制（super\_admin / admin）
- 完整 API 文档（`API.md`），含接口说明、参数、响应示例

**UI/UX 优化**

- 仪表盘采用高对比度渐变卡片设计
- 统计卡片使用深色渐变背景，文字清晰可读
- 修复 echarts 初始化 TypeError 错误
- 前台网页添加心跳自动刷新（每30秒）
...
- 新增 Vue 3 + Element Plus 后台管理面板
- 支持 JWT 管理员登录认证（bcrypt 加密）
- 仪表盘：图片统计、今日上传、分类数量、系统状态
- 图片管理：列表查看、搜索、删除、批量删除、修改分类
- 分类管理：创建、编辑、删除、排序
- 标签规则：自动分类标签匹配规则管理
- 系统设置：上传限制、格式配置、功能开关
- 管理员管理：创建、编辑、禁用、删除（仅超级管理员）
- 操作日志：登录、删除、修改等全量操作记录
...
详细 API 文档请参考 [API.md](API.md)，包含以下模块：

| 模块 | 接口数 | 说明 |
|------|--------|------|
| 认证接口 | 3 | 登录、登出、获取用户信息 |
| 分类管理 | 6 | 增删改查、批量删除、图片关联检查 |
| 图片管理 | 8 | 上传、删除、批量删除、资源清理 |
| 管理员接口 | 2 | 仪表盘数据、心跳检测 |
| 系统接口 | 1 | 健康检查 |
